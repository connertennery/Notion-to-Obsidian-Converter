...
const correctMarkdownLinks = (content) => {
  ...
  if (linkFullMatches) {
    ...
    for (let i = 0; i < linkFullMatches.length; i++) {
      ...
      const linkTextMatches = linkFullMatches[i].match(/\[(.*?)\]\((.*?)\)/i);
      let linkText = tryDecodeURI(linkTextMatches[1]);
      let linkDecoded = tryDecodeURI(linkTextMatches[2]);
      const pngMatch = linkDecoded.match(/^(.*?) \w{32}\/(.*?)\.png$/);
      if (pngMatch) {
        vlog(4, `Fixing png link: ${pngMatch[1] + "/" + pngMatch[2]}`);
        linkText = pngMatch[1] + "/" + pngMatch[2] + ".png";
      } else {
