...
`
  ...
  console.warn(
    `\t• Please make a backup of your export and read the warnings in the README so your data isn't mangled!`
  );
  console.warn(
    `\t• Notion pages that contain parentheses or dashes in the title will have them removed by Notion while exporting your data so the file will be created without them, even though the link itself will still retain them.`
  );
  console.warn(
      ...
      if (exists) {
        ...
        if (isDir) {
          if (exportPath === undefined) {
            ...
            console.warn(
              `Provided multiple paths - right now the converter can only operate on one directory and its subdirectories`
            );
            process.exit(1);
          }
        } else {
...
function main() {
  ...
  if (process.argv.includes(`-vvv`)) flags.logging = 4;
...
const correctMarkdownLinks = (content: string) => {
  ...
  if (linkFullMatches) {
    ...
    for (let i = 0; i < linkFullMatches.length; i++) {
      ...
      let linkText = linkDecoded.substring(1, linkDecoded.length - 2);
      ...
      if (linkText[0] === "[") out = out.replace(linkFullMatches[i], linkText);
      else out = out.replace(linkFullMatches[i], `[[${linkText}]]`);
    }
  }
...
const convertRelativePath = (path: string) => {
  ...
  return `[[${tryDecodeURI(
    path.split("/").pop()!.split("%20").slice(0, -1).join(" ")
  )}]]`;
};
...
const convertCSVToMarkdown = (content: string) => {
  ...
  const headersplit =
    "|" +
    "---|".repeat(fix[0].split("").filter((char) => char === "|").length - 1);
  fix.splice(1, 0, headersplit);
  return fix.join("\n");
};

const convertDirectory = function (path: string) {
...
function startConversion(path: string) {
...
function tryDecodeURI(encoded: string) {
...
function vlog(level: number, message: string) {
  if (flags.logging >= level) console.log(message);
}

function verror(level: number, message: string) {
  if (flags.logging >= level) console.error(message);
}
